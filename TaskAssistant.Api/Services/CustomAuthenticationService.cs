...
namespace TaskAssistant.Api.Services
{
    public class CustomAuthenticationService : ICustomAuthenticationService
    {
        ...
        public CustomAuthenticationService(ApplicationDbContext context, UserManager<IdentityUser> userManager, IOptions<AuthOptions> authOptions)
        {
            _context = context;
        ...
        public async Task<bool> ValidateUser(Login loginModel)
        {
            var user = await _userManager.FindByNameAsync(loginModel.UserName);
            return await _userManager.CheckPasswordAsync(user, loginModel.Password);
        }

        public async Task<dynamic> BuildJwtToken(string userName, int tokenExpiryTime)
        {
